...
class Saweria {
    ...
    async createPayment(amount, msg = 'Order') {
        ...
        try {
            const payload = {
                agree: true, amount: Number(amount),
                customer_info: { first_name: 'fandirr', email: "fandirrstore@gmail.com", phone: '' },
                message: msg, notUnderAge: true, payment_type: 'qris', vote: ''
            };
            // FUNGSI INTERNAL TETAP POST
...
// ENDPOINT KITA MENGGUNAKAN GET
app.get('/saweria/login', async (req, res) => {
    const { email, password } = req.query;
    if (!email || !password) return res.status(400).json({ status: false, message: 'Parameter email dan password wajib diisi.' });
    try {
...
app.get('/saweria/payment/create', async (req, res) => {
    ...
    if (!user_id || !amount) return res.status(400).json({ status: false, message: 'Parameter user_id dan amount wajib diisi.' });
    try {
...
app.get('/saweria/cekstatus', async (req, res) => {
    ...
    if (!id || !user_id) return res.status(400).json({ status: false, message: 'Parameter "id" dan "user_id" wajib diisi.' });
    try {
        const saweria = new Saweria(user_id);
